...
class Res_Ajax_model extends CI_Model
{
	
	public function createData($data)
	{
		$query = $this->db->insert('ResidentTBL', $data);
		return $query;
	}
	...
	public function fetchAllData($data, $tablename, $where)
	{
		$query = $this->db->select($data)
			->from($tablename)
			->where($where)
			 ->order_by('ResidentID', 'DESC') 
			->get();
		return $query->result_array();
	}
		public function fetchAllDataPay($data, $tablename, $where)
	{
		$query = $this->db->select($data)
			->from($tablename)
			->where($where)
